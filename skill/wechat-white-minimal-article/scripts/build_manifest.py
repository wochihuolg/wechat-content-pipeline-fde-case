...
def main() -> int:
    ...
    for article_dir in article_dirs:
        ...
        try:
            ...
            original_title = instruction_title(text)
        ...
        if not isinstance(draft_title, str) or not draft_title.strip():
            errors.append(f"{article_dir.name}: missing public title")
            continue
        draft_title = draft_title.strip()
        if original_title != draft_title:
            errors.append(f"{article_dir.name}: title map does not match 贴图指令.md")
        if not draft_title.startswith(PUBLIC_TITLE_PREFIX) or not draft_title[len(PUBLIC_TITLE_PREFIX):].strip():
            errors.append(f"{article_dir.name}: title must use the complete column formula")
        if len(draft_title) > MAX_PUBLIC_TITLE_LENGTH:
            errors.append(f"{article_dir.name}: title has {len(draft_title)} characters; max is 32")
