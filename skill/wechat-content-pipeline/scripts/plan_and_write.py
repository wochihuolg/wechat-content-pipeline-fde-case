...
def validate_topics(payload: dict[str, Any], count: int, max_ref: int) -> list[dict[str, Any]]:
    ...
    for position, item in enumerate(topics, start=1):
        ...
        if len(compact_title) > 20:
            raise ValueError(f"Topic {position} compact_title exceeds 20 characters.")
...
def plan_topics(
    ...
    system_prompt = """You are a Chinese WeChat content strategist. Build non-duplicative,
source-grounded topics for native six-image posts. Independently synthesize ideas; do not copy
source wording, layouts, screenshots, watermarks, or unsupported claims. Return JSON only."""
    schema = {
        "topics": [
            {
                "index": 1,
                "slug": "lowercase_english_slug",
                "title": "high-click Chinese working title",
                "compact_title": "20 Chinese characters or fewer",
...
- compact_title must be at most 20 Chinese characters.
...
def write_article(
    ...
    system_prompt = """You are a Chinese WeChat writer and editorial designer. Independently
rewrite source-grounded knowledge as a concise, practical article plus a six-image native post.
...
Audience: {audience}
Title: {topic['title']}
Compact title: {topic['compact_title']}
